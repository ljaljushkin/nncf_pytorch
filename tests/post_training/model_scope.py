...
WEIGHT_COMPRESSION_MODELS = [
    ...
    },
    {
        ...
        "compression_params": {
            ...
            "ignored_scope": nncf.IgnoredScope(patterns=[".*embed.*"]),
            # "ignored_scope": nncf.IgnoredScope(patterns=["(?!.*lm_head.*)\w+"]),  # ".*lm_head.*"]),
            # "group_size": 128,
    ...
    },
    {
        ...
        "compression_params": {
            "mode": CompressWeightsMode.INT8_ASYM,
            "ignored_scope": nncf.IgnoredScope(patterns=["(?!.*embed.*)\w+"]),  # ".*lm_head.*"]),
            # "ignored_scope": nncf.IgnoredScope(patterns=[".*embedding.*"]),  # ".*lm_head.*"]),
            # "group_size": 64,
            # "ratio": 0.8,
            # "mode": CompressWeightsMode.INT4_SYM,
            # "sensitivity_metric": SensitivityMetric.WEIGHT_QUANTIZATION_ERROR,
