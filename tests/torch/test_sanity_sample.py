...
class SampleTestDescriptor(ABC):
    def __init__(self):
    ...
    def finalize(self, dataset_dir=None) -> 'SampleTestDescriptor':
...
class PrecisionTestDescriptor(SampleTestDescriptor):
    def __init__(self):
        super().__init__()
...
class HAWQDescriptorSample(PrecisionTestDescriptor):
    def __init__(self):
        super().__init__()
...
class AutoQDescriptorSample(PrecisionTestDescriptor):
    def __init__(self):
        super().__init__()
...
def resnet18_desc(x: PrecisionTestDescriptor):
...
def inception_v3_desc(x: PrecisionTestDescriptor):
...
def ssd300_vgg_desc(x: PrecisionTestDescriptor):
...
def unet_desc(x: PrecisionTestDescriptor):
...
def icnet_desc(x: PrecisionTestDescriptor):
...
TEST_CASE_DESCRIPTORS = [
    inception_v3_desc(HAWQDescriptorSample()),
    inception_v3_desc(HAWQDescriptorSample()).staged(),
    resnet18_desc(HAWQDescriptorSample()),
    resnet18_desc(HAWQDescriptorSample()).staged(),
    resnet18_desc(HAWQDescriptorSample().batch_for_init(2)),
    resnet18_desc(HAWQDescriptorSample().batch_for_init(2)).staged(),
    ssd300_vgg_desc(HAWQDescriptorSample()),
    ssd300_vgg_desc(HAWQDescriptorSample().batch_for_init(2)),
    unet_desc(HAWQDescriptorSample()),
    unet_desc(HAWQDescriptorSample().batch_for_init(2)),
    icnet_desc(HAWQDescriptorSample()),
    inception_v3_desc(AutoQDescriptorSample()).batch(2),
    inception_v3_desc(AutoQDescriptorSample()).staged(),
    resnet18_desc(AutoQDescriptorSample()).batch(2),
    resnet18_desc(AutoQDescriptorSample().dump_debug(True)).batch(2).staged(),
    resnet18_desc(AutoQDescriptorSample().subset_ratio(0.2)).batch(2),
    resnet18_desc(AutoQDescriptorSample().subset_ratio(0.2)).staged(),
    ssd300_vgg_desc(AutoQDescriptorSample().dump_debug(True)).batch(2),
    unet_desc(AutoQDescriptorSample().dump_debug(True)),
    icnet_desc(AutoQDescriptorSample())
]


@pytest.fixture(params=TEST_CASE_DESCRIPTORS, ids=[str(d) for d in TEST_CASE_DESCRIPTORS])
def desc(request, dataset_dir):
    desc: PrecisionTestDescriptor = request.param
    return desc.finalize(dataset_dir)


def validate_sample(args, desc: SampleTestDescriptor, mocker):
    arg_list = [key if (val is None or val is True) else "{} {}".format(key, val) for key, val in args.items()]
    command_line = " ".join(arg_list)
...
def test_precision_init(desc: PrecisionTestDescriptor, tmp_path, mocker):
...
class ExportDescriptorSample(PrecisionTestDescriptor):
    def __init__(self):
        super().__init__()
...
EXPORT_TEST_CASE_DESCRIPTORS = [
    resnet18_desc(ExportDescriptorSample()),
    resnet18_desc(ExportDescriptorSample()).staged(),
    ssd300_vgg_desc(ExportDescriptorSample()),
    unet_desc(ExportDescriptorSample()),
]
...
def test_export_behavior(export_desc: PrecisionTestDescriptor, tmp_path, mocker, extra_args, is_export_called):
