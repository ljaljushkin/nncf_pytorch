...
from nncf.common.graph.layer_attributes import MultipleInputLayerAttributes, PermuteLayerAttributes, TransposeLayerAttributes
...
def get_model_with_transpose_and_permute(batch_size=None):
    ...
    x = tf.transpose(inputs, [0, 3, 2, 1, 4])
...
def test_permute_attribute_saved_during_graph_building():
    ...
    ref_split_nodes = {'tf.Transpose': PermuteLayerAttributes([3, 0, 2, 1, 4]),
                       'tf.Permute': PermuteLayerAttributes([3, 0, 2, 1, 4]),}
    for node in graph.get_all_nodes():
