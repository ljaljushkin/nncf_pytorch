...
This example demonstrates how to improve accuracy of Large Language Models (LLMs) with 4bit weights by quantization-aware-training with absorbable LoRA adapters.
...
```bash
CUDA_VISIBLE_DEVICES=0 ./tune.sh
```
...
```bash
CUDA_VISIBLE_DEVICES=0 ./tune_smolm.sh
```
