...
def wwb_eval(model_id, ckpt_dir, file_handle):
    cmd = f"python wwb_eval_no_chat.py -m={model_id} -n={ckpt_dir}"
    sys.stdout.flush()
    subprocess.run(cmd.split(" "), stdout=file_handle, stderr=file_handle)

    result_path = ckpt_dir / "results_wwb_no_chat.json"
...
def finetune(
    ...
    for epoch in range(args.epochs):
        ...
        if smlr > metadata["best_similarity"]:
            ...
            shutil.copy(last_dir / "results_wwb_no_chat.json", best_wwb_dir / "results_wwb_no_chat.json")
