...
def load_state(model: torch.nn.Module, state_dict_to_load: dict, is_resume: bool = False,
               ignored_keys: List[str] = None) -> int:
    """
    ...
    key_matcher = KeyMatcher(is_resume, state_dict_to_load, model_state_dict, ignored_keys)
...
class NormalizedKeys:
        ...
        for orig_key in keys:
            ...
            for normalized_key in normalized_keys:
                if normalized_key in ignored_keys:
                    self.ignored_orig_keys.append(orig_key)
                    continue
    ...
    def get_ignored_orig_keys(self):
        return self.ignored_orig_keys

    def get_orig_key(self, normalized_key: str):
        return self._norm_key_vs_orig_key_map[normalized_key]

    def _key_normalizer(self, key: str) -> List[str]:
        new_key = key
        ...
        new_key, did_replace = NormalizedKeys._replace_legacy_act_quantizer_storage_name(new_key)
        if did_replace:
            self.has_legacy_storage_keys = True

        result = NormalizedKeys._split_unified_parameters(new_key)
        if len(result) > 1:
            self.is_unified_group_detected = True
        return result
...
class KeyMatcher:
    ...
    def run(self) -> Dict[str, torch.Tensor]:
        ...
        """
        normalized_model_keys = NormalizedKeys(list(self.model_state_dict.keys()), self.ignored_keys)
        normalized_keys_to_load = NormalizedKeys(list(self.state_dict_to_load.keys()), self.ignored_keys)
