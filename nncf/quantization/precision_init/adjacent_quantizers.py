...
class GroupsOfAdjacentQuantizers:
    """
    Contains groups of adjacent quantizer
    ...
    def __init__(self):
        # TODO: where to store? in AdjacentQuantizer?
        ...
        for group_idx, group in enumerate(quantizer_setup.shared_input_operation_set_groups):
            ...
            for module_scope, w_qp_id in module_scope_per_weight_qp_id.items():
                if module_scope not in module_scope_per_activation_qp_id:
                    nncf_logger.warning(f'Not quantized inputs for module `{module_scope}` with quantized weights!')
                    continue
                    # TODO: raise RuntimeError(f'Not quantized inputs for module `{module_scope}` with quantized weights!')
                a_qp_id = module_scope_per_activation_qp_id[module_scope]
                if w_qp_id in self.weight_qp_id_per_activation_qp_id:
                    nncf_logger.warning(f'Multiple weight quantizers per activation quantizer for `{module_scope}`')
                    continue
                    # TODO: raise RuntimeError(f'Not quantized inputs for module `{module_scope}` with quantized weights!')
